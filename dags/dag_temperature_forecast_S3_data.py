...
def train_in(**kwargs):
    ...
    key_to_save_s3 = f'models/{kwargs["ds"]}_train_in_data.pkl'
...
def train_out(**kwargs):
    ...
    serialized_model = codecs.encode(pickle.dumps(m), "base64").decode()
    key_to_save_s3 = f'models/{kwargs["ds"]}_train_out_data.pkl'
    s3_hook.load_string(serialized_model, key_to_save_s3, "airflow", replace=True)
...
with DAG(
    ...
    dag_id='Temperature_Forecast_S3_Data',
    ...
    final_task = TriggerDagRunOperator(
        trigger_dag_id='docker_dag_data',
        task_id='get_model'
    )
